          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="relative group">
                <pre class="bg-neutral-800 text-pink-400 p-3 pr-10 rounded break-all whitespace-pre-wrap">{entry.pattern}</pre>
                <button
                  aria-label="Copy regex"
                  on:click={() => copy(entry.pattern, `${entry.title}-find`)}
                  class={
                    `absolute top-1.5 right-1.5 p-1 rounded transition-opacity duration-200 backdrop-blur-sm
                ...
                >
                  <Icon icon={copiedKey === `${entry.title}-find` ? 'mdi:check' : 'mdi:content-copy'} class="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            {#if entry.replace}
              <div>
                ...
                <div class="relative group">
                  <pre class="bg-neutral-800 text-green-400 p-3 pr-10 rounded break-all whitespace-pre-wrap">{entry.replace}</pre>
                  <button
                  ...
                  >
                    <Icon icon={copiedKey === `${entry.title}-replace` ? 'mdi:check' : 'mdi:content-copy'} class="w-4 h-4" />
                  </button>
                </div>
              </div>
            {/if}
